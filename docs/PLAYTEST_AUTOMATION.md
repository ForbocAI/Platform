...
**Autoplay-driven verification (2026-02-20):** Resolved "Visual Log Stall" and "Hydration Failed" errors. Refactored `addLog` to use deterministic `prepare` callback; removed non-deterministic state updates (Date.now, Math.random) from all reducers. Verified via 40s autoplay soak: logs grew from 4 to 14+ entries; equipment actions, scans, and combat correctly logged without UI stall. Fallback mode (no-sdk) verified as fully stable.
...
| **COMMUNE** | Click "Perform Inquiry with void" | ✅ Log "You attempt to commune with the void..."; Oracle answer and facts added |
...
| **Soul Re-hydration** | Inject `soulId` (Arweave data item ID) into room enemy data. Verify NPC/Agent hydrate correctly. | ✅ **Implemented:** `rehydrateAgent` thunk support. |
...
4. **Hazard "Threat Imminent":** Be in any room, click "Perform Inquiry with void" repeatedly until the Oracle returns an answer with "[EVENT: Entering the Red]". SCAN or check room view to see "Threat Imminent" in hazards (or an enemy may be added instead, 50% each).
5. **Fellow Ranger (Enter Stage Left):** Be in a room without a Fellow Ranger, click "Perform Inquiry with void" until the Oracle returns "[EVENT: Enter Stage Left]". RoomViewport and SCAN will show "Fellow Ranger".
