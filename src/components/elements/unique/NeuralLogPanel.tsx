...
export function NeuralLogPanel({ logs, children }: { logs: GameLogEntry[]; children?: React.ReactNode }) {
  return (
    <aside className="vengeance-border bg-zinc-950 flex flex-col min-h-[6rem] h-32 sm:h-36 lg:h-full lg:min-h-0 lg:border-l-0 w-full min-w-0">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-1.5 sm:p-2 lg:p-4 space-y-1 sm:space-y-2 lg:space-y-4 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent min-h-0 min-w-0">
        {logs.map((log) => (
          <div key={log.id} className="text-[9px] sm:text-[10px] lg:text-xs min-w-0 break-words">
            <p
              className={cn(
